...
declare module 'excel2all' {
	 ...
	 */
	function forEachFile(fileOrDirPath: string, eachCallback?: (filePath: string) => void): void;
	/**
	 * 批量写入和删除文件
	 ...
	 * @param complete 完成回调
	 */
	function writeOrDeleteOutPutFiles(outputFileInfos: IOutPutFileInfo[], onProgress?: (filePath: string, total: number, now: number, isOk: boolean) => void, complete?: (total: number) => void): void;
	/**
	 * 获取变化过的文件数组
	 * @param dir 目标目录
	 ...
	 * @returns 返回缓存文件路径
	 */
	function forEachChangedFile(dir: string, cacheFilePath?: string, eachCallback?: (filePath: string, isDelete?: boolean) => void): void;
	/**
	 * 写入缓存数据
	 * @param cacheFilePath
	 * @param cacheData
	 */
	function writeCacheData(cacheFilePath: string, cacheData: any): void;
	/**
	 * 读取缓存数据
	 * @param cacheFilePath
	 */
	function getCacheData(cacheFilePath: string): any;
	/**
	 * 获取文件md5 (同步)
	 * @param filePath
	 */
	function getFileMd5Sync(filePath: string): string;
	/**
	 * 获取文件 md5
	 * @param filePath
	 */
...
declare module 'excel2all' {
	function doParse(parseConfig: ITableConvertConfig, fileInfos: IFileInfo[], parseResultMap: TableParseResultMap, parseHandler: ITableParseHandler): void;
...
declare module 'excel2all' {
	 ...
	 * @param charCodes
	 */
	function getNextColKey(charCodes: number[]): string;
	/**
	 * 列的字符编码数组转字符串
	 * @param charCodes
	 */
	 ...
	 * @param colKey
	 */
	function stringToCharCodes(colKey: string): any[];
	/**
	 * 纵向遍历表格
	 ...
	 */
	function verticalForEachSheet(sheet: xlsx.Sheet, startRow: number, startCol: string, callback: (sheet: xlsx.Sheet, colKey: string, rowIndex: number) => void, isSheetRowEnd?: (sheet: xlsx.Sheet, rowIndex: number) => boolean, isSheetColEnd?: (sheet: xlsx.Sheet, colkey: string) => boolean, isSkipSheetRow?: (sheet: xlsx.Sheet, rowIndex: number) => boolean, isSkipSheetCol?: (sheet: xlsx.Sheet, colKey: string) => boolean): void;
	/**
	 * 横向遍历表格
	 * @param sheet xlsx表格对象
	 * @param startRow 开始行 从1开始
	 ...
	 * @param isSkipSheetCol 是否跳过列
	 */
	function horizontalForEachSheet(sheet: xlsx.Sheet, startRow: number, startCol: string, callback: (sheet: xlsx.Sheet, colKey: string, rowIndex: number) => void, isSheetRowEnd?: (sheet: xlsx.Sheet, rowIndex: number) => boolean, isSheetColEnd?: (sheet: xlsx.Sheet, colkey: string) => boolean, isSkipSheetRow?: (sheet: xlsx.Sheet, rowIndex: number) => boolean, isSkipSheetCol?: (sheet: xlsx.Sheet, colKey: string) => boolean): void;
	/**
	 * 读取配置表文件 同步的
	 * @param fileInfo
	 */
	function readTableFile(fileInfo: IFileInfo): xlsx.WorkBook;
	/**
	 * 根据文件名后缀判断是否为csv文件
	 * @param fileExtName
	 */
	function isCSV(fileExtName: string): boolean;
...
declare module 'excel2all' {
	...
	class DefaultParseHandler implements ITableParseHandler {
...
declare module 'excel2all' {
	class DefaultConvertHook implements IConvertHook {
...
declare module 'excel2all' {
	 ...
	 */
	function convert(converConfig: ITableConvertConfig): Promise<void>;
	/**
	 * 测试文件匹配
	 * @param converConfig
	 */
	function testFileMatch(converConfig: ITableConvertConfig): void;
...
declare namespace excel2all {
	type Logger = import('excel2all').Logger;
	type DefaultParseHandler = import('excel2all').DefaultParseHandler;
	type DefaultOutPutTransformer = import('excel2all').DefaultOutPutTransformer;
	type DefaultConvertHook = import('excel2all').DefaultConvertHook;
}
declare const excel2all:typeof import("excel2all");