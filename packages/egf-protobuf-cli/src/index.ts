...
export async function generate(projRootDir: string) {
    ...
    if (pbconfig.concatPbjsLib) {
        pbjsLibFileStr = (await fs
            .readFileAsync(path.join(root, `pblib/${libType}/protobuf.min.js`))
            .catch(function (res) {
                console.log(res);
            })) as any;
    }
    ...
    let outPbj =
        (pbconfig.concatPbjsLib ? pbjsLibFileStr : "") +
    ...
    if (pbconfig.outputFileType === 0 || pbconfig.outputFileType === 1) {
        ...
        await fs.writeFileAsync(clientPbjsFilePath, outPbj, "utf-8").catch(function (res) {
            console.log(res);
        });
    ...
    if (pbconfig.outputFileType === 0 || pbconfig.outputFileType === 2) {
        ...
        await fs.writeFileAsync(clientPbjsFilePath, minjs, "utf-8").catch(function (res) {
            console.log(res);
        });
    ...
    if (serverPbjsFilePath) {
        ...
        await fs.writeFileAsync(serverPbjsFilePath, outPbj, "utf-8").catch(function (res) {
            console.log(res);
        });
    ...
    for (let i = 0; i < dtsOutFilePaths.length; i++) {
        ...
        await fs.writeFileAsync(dtsOutFilePath, pbtsResult, "utf-8").catch(function (res) {
            console.log(res);
        });
    }
...
export async function initProj(projRoot: string = ".", projType: string) {
    ...
    await fs.copyAsync(path.join(root, "pblib"), protoLibDirPath).catch(function (res) {
        console.log(res);
    });
